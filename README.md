...
- **오늘의 명언** — 내장된 세계 명언 365선을 매일 하나씩 자동 교체 표시 (1년 내내 겹치지 않음, 오프라인 동작, 폰트 선택·토글 가능)
- **날씨** — 도시를 검색해 선택하면 현재 날씨·기온·최저/최고를 표시 (Open-Meteo 무료 API, 키 불필요, 30분 자동 갱신)

### 창 · 시스템
- 프레임 없는 투명 창, **Ctrl + 드래그로 이동**(호버 시 하단에 안내 표시), 가장자리 드래그로 크기 조절, 호버 시 테두리 하이라이트
- **우클릭 메뉴** — 설정/항상 위/최소화/트레이로 최소화/닫기 (작은 창에서는 호버 버튼 대신 사용)
- **트레이 아이콘** — 왼쪽 클릭으로 시계 숨기기/보이기 토글, 우클릭 메뉴(열기/종료)
